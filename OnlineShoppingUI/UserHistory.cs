...
namespace OnlineShoppingUI
{
    internal class UserHistory
    {
        ...
        public void AddAction(string action)
        {
            ...
            {
                db.HistoryRecords.Add(new HistoryRecord
                {
                    Action = action,
        ...
        public List<string> GetAllActions()
        {
            ...
            using (var db = new AppDbContext())
            {
                var records = db.HistoryRecords
                                .OrderByDescending(r => r.Timestamp)
                                .ToList();

                foreach (var record in records)
                {
                    actions.Add($"[{record.Timestamp}] {record.Action}");
                }
            }
            HistoryNode current = head;
            while (current != null)
            {
                actions.Add($"[{current.Timestamp}] {current.Action}");
                current = current.Next;
            }
            return actions;
        }
